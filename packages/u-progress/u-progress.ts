import { IS_IOS, UHTMLElement, attr, createElement, customElements, getRoot, useId } from '../utils'
...
export class UHTMLProgressElement extends UHTMLElement {
  ...
  constructor() {
    ...
    this.attachShadow({ mode: 'closed' }).append(
      createElement('slot', { hidden: true }), // Slot to hide content meant for legacy user agents https://html.spec.whatwg.org/multipage/form-elements.html#the-progress-element
      createElement('style', {
        textContent:
  ...
  attributeChangedCallback() {
    ...
    attr(this, {
      [IS_IOS ? 'aria-description' : 'aria-valuenow']: IS_IOS ? `${percentage}%` : percentage,
      'aria-busy': indeterminate || null,
      'aria-labelledby': attr(this, 'aria-label') ? null : useId(this.labels[0]), // TODO Improve
      'aria-valuemax': 100,
      'aria-valuemin': 0,
  ...
  get labels(): NodeListOf<HTMLLabelElement> {
    ...
    const labels = getRoot(this).querySelectorAll<HTMLLabelElement>(`[for="${useId(this)}"]`)
