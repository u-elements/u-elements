...
import { UHTMLTabsElement } from './u-tabs';
...
describe('u-tabs', () => {
  ...
  test('accepts 0 as valid value to select first tab by index', async ({ page }) => {
    ...
    `)
    await coreTabs.evaluate((node: UHTMLTabsElement) => node.selectedIndex = 0)
    await expect(page.getByText('First tab')).toHaveAttribute('aria-selected', 'true')
    await expect(page.getByText('Second tab')).toHaveAttribute('aria-selected', 'false')
  ...
  //     <div id="panel-2">Text of tab 2</div>
  //   `)
  //   await coreTabs.evaluate((node: UHTMLTabsElement) => node.selectedIndex = 'tab-2')
  ...
  //     <div id="panel-2">Text of tab 2</div>
  //   `)
  //   await coreTabs.evaluate((node: selectedIndex) => node.selectedIndex = node.querySelector('#tab-2') as TabElement)
  //   await expect(page.getByText('First tab')).toHaveAttribute('aria-selected', 'false')
  //   await expect(page.getByText('Second tab')).toHaveAttribute('aria-selected', 'true')
  // })
  ...
  test('handles setup and interaction with dynamically added tabs', async ({ page }) => {
    await page.setContent(`
      ...
      <div id="panel-2">Text of tab 2</div>
    `)
    await coreTabs.evaluate((node: UHTMLTabsElement) => node.appendChild(Object.assign(document.createElement('button'), { id: 'tab-1', textContent: 'First tab' })))
    await coreTabs.evaluate((node: UHTMLTabsElement) => node.appendChild(Object.assign(document.createElement('button'), { id: 'tab-2', textContent: 'Second tab' })))
  ...
  test.describe('supports optional attribute "tab"', () => {
    ...
    //     expect(await coreTabs.evaluate((node: UHTMLTabsElement) => (node.tab as TabElement).id)).toEqual('tab-3')
    //   })
    //   await test.step('updates tab attr', async () => {
    //     await coreTabs.getByRole('tab', { name: 'Second Tab' }).click()
    //     await expect(coreTabs).toHaveAttribute('tab', '1')
    //     expect(await coreTabs.evaluate((node: UHTMLTabsElement) => (node.tab as TabElement).id)).toEqual('tab-2')
    //   })
    // })
    ...
    //     expect(await coreTabs.evaluate((node: UHTMLTabsElement) => (node.tab as TabElement).id)).toEqual('tab-1')
    //   })
    //   await test.step('updates tab attr', async () => {        
    //     await coreTabs.getByRole('tab', { name: 'Second Tab' }).click()
    //     await expect(coreTabs).toHaveAttribute('tab', '1')
  ...
  test.describe('supports fewer panels than tabs, e.g used with dynamic content', () => {
    ...
    //   })
    //   const { panels, tabs, panel, tab } = await coreTabs.evaluate(({ panels, tabs, panel, tab }: UHTMLTabsElement) => ({ panels, tabs, panel, tab: (tab as TabElement).id }))
    ...
    test('assigns aria-labelledBy to match the selected tab', async ({ page }) => {
      await page.setContent(`
        <u-tabs id="dynamicInstance" tab="tab-2">
          <button id="tab-1" data-for="onlyPanel">First tab</button>
          <button id="tab-2" data-for="onlyPanel">Second tab</button>
          <button id="tab-3" data-for="onlyPanel">Third tab</button>
        </u-tabs>
